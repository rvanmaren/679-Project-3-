...
function Level(levelNum)
{
	...
	for(var i=0; i< this.totalZombies; i++)
	{
		...
		if(Math.random() > .5){
			location.x = this.randomBetween(2500, 4000); 
		} else{
			location.x = this.randomBetween(6000, 7500);
		}

		if(Math.random() > .5){
			location.z = this.randomBetween(2500, 4000); 
		} else{
			location.z = this.randomBetween(6000, 7500);
		}
		if(levelNum == 1){
			...
			if(pick > Math.min(.3 + (.025 *levelNum), .8)){
			    this.zombies.push(new Skeleton(location));
			} else if (pick > Math.min(.02 + .01 * levelNum,.15)){
				this.zombies.push(new Monster(location));
			}
			else
			{
